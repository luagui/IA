...
public class GeneticDemo {
	...
	private static void myGeneticAlgorithmSearch() {
		...
		try {
			...
			GoalTest<Individual<Integer>> goalTest = GeneticAlgoUtilAlgoUtil.getGoalTest();
			...
			GeneticAlgorithm<Integer> ga = new GeneticAlgorithm<>(arraySize,
					NQueensGenAlgoUtil.getFiniteAlphabetForBoardOfSize(arraySize), 0.15);
			...
			System.out.println("# Board Layouts = " + (new BigDecimal(arraySize)).pow(arraySize));
